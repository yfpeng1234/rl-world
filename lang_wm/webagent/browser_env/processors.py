...
class ObservationProcessor:
    def process(self, page: Page, client: CDPSession) -> Observation | tuple[Observation, ...]:
        raise NotImplementedError


class ObservationMetadata(TypedDict):
    obs_nodes_info: dict[str, Any]
    obs_nodes_semantic_info: dict[str, Any]


def create_empty_metadata() -> ObservationMetadata:
    return {
        "obs_nodes_info": {},
        "obs_nodes_semantic_info": {}
    }
...
class TextObervationProcessor(ObservationProcessor):
    ...
    def process(self, page: Page, client: CDPSession) -> str:
        ...
        try:
            ...
            page.wait_for_load_state("load", timeout=250000)
            browser_info = self.fetch_browser_info(page, client)
...
class ImageObservationProcessor(ObservationProcessor):
    ...
    def process(self, page: Page, client: CDPSession) -> tuple[npt.NDArray[np.uint8], npt.NDArray[np.uint8], str]:
        try:
            browser_info = self.fetch_browser_info(page, client)
        except Exception:
            page.wait_for_load_state("load", timeout=250000)
