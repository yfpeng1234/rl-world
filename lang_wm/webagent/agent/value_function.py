...
"""
    ...
    for r_idx, r in enumerate(all_responses):
        ...
        def _search(pattern: AnyStr, string: AnyStr | None) -> AnyStr:
            assert string is not None and (
                res := re.search(pattern, string)
            ) is not None
            return res.group(1)

        try:
            pred = _search(r'Status: "?(.+)"?', r.message.content)
            if 'failure' in pred.lower():
                score = 0.0
            else:
                # Check if it's on the path to success
                on_path = _search(r'On the right track to success: "?(.+)"?', r.message.content)
