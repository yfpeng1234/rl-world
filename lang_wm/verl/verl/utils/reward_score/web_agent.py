...
def hungarian_match(A, B, p):
    """
    A should be the ground truth
    len(A) is used for normalization instead of len(B)
    otherwise the model may learn something nasty, namely generating long items

    A = {"Tommy is a lovely dog", "Jacob is my friend", "Brain is my classmate", "Sandy"}
    B = {"Amy? who are you", "Jason is my friend", "Tom is a lovely dog", "Sandy", "Tommy is a dog"}
    ...
    for i, a in enumerate(A):
        for j, b in enumerate(B):
            ...
            #     type_b = ""
            #
            # cost = distance(a, b) / len(a)
            # cost += 0.5 * (index_a != index_b)
            # cost += 0.5 * ((type_a != type_b) or (type_a == "") or (type_b == ""))
            # if cost <= p:
...
def compute_score(solution_str, ground_truth, p=0.1):
    """
    ...
    for section in solution_dict:
        reward += hungarian_match(solution_dict[section], ground_truth_dict[section], p)
        if len(solution_dict[section]) == 0 and len(ground_truth_dict[section]) == 0:
            reward += 1
    return float(reward)


def compute_score_(solution_str, ground_truth, p=0.1):
    """
    solution_str: predicted string
    ground_truth: ground truth strings
    p: strictness for matching, should be in [0, 1]
...
if __name__ == '__main__':
    ...
    # print(compute_score(log_text, log_text_, 0.1))
    # print(compute_score(log_text, log_text, 0.1))
    print(compute_score_(log_text, log_text, 0.1))
    print(compute_score_(log_text, log_text_, 0.1))
    # A = {"a11", "a2", "a3"}
    # B = {"b11", "a1"}
    # print(hungarian_match(A, B, 0.50))  # 2
    # print(hungarian_match(A, B, 0.40))  # 1