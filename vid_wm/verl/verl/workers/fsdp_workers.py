...
class TokenizerWorker(Worker):
    ...
    def detokenize(self, data: DataProto, lpips_data: DataProto):
        ...
        if 'lpips' in lpips_data.meta_info.keys() and lpips_data.meta_info['lpips']:
            if 'real' not in lpips_data.batch.keys():
                ...
                real = lpips_data.batch['real'].to(torch.cuda.current_device())
                # assert (real == self.cached_pixels[:, 2:]).all()
    ...
    def process(self, data: DataProto):
        device = torch.cuda.current_device()
        pixels, actions = data.batch['pixels'].to(device), data.batch['actions'].to(device)
        self.cached_pixels = pixels
        if self.config.tokenizer.name == 'ctx_cnn':
            output, ctx_tokens = self.processor(pixels, actions, return_ctx_tokens=True)
            output['ctx_tokens'] = ctx_tokens
        else:
            output = self.processor(pixels, actions)
