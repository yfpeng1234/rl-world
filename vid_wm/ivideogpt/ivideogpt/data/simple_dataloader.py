...
class SimpleRoboticDatasetv2(data.Dataset):
    def __init__(
        ...
        if dataset_name == 'bair_robot_pushing':
            ...
            if task_instruction is not None:
                all_task_instruction = np.load('task_instruction.npy', allow_pickle=True)
                self.filenames = [x for i, x in enumerate(self.filenames) if all_task_instruction[i] == task_instruction]
            if train:
    ...
    def get_segment(self, episode, action=None):
        ...
        if self.random_ctx_frame:
            ctx_index = np.random.randint(max(start-15, 0), start+1)
            images = [episode[ctx_index]] + images
            if action is not None:
                actions = [action[ctx_index]] + actions
    ...
    def data_augmentation(self, images):
        ...
        for image in images:
            image = F.resized_crop(image, i, j, h, w, self.image_size)
    ...
    def centercrop(self, images):
        ...
        return F.resize(images, self.image_size)
    ...
    def __getitem__(self, item):
        while True:
            ...
            if len(episode) >= self.segment_length:
                break
        ...
        returns = [images]
        if self.load_action:
            returns.append(torch.Tensor(np.array(actions)))
        if len(returns) == 1:
            return returns[0]
...
class EvalDataset(data.Dataset):
    def __init__(
        ...
        self.filenames = glob.glob(os.path.join(parent_dir, self.dataset_name, '*.npz'))
        self.filenames.sort()
        self.filenames = [x for i, x in enumerate(self.filenames) if i % 100 == 0]

        self.size = len(self.filenames)
        self.display_key = get_display_key(self.dataset_name)
        if self.size == 0:
            raise ValueError(f"[EvalDataset] No test episodes found in {self.dataset_name}")
        print(f"[EvalDataset] Find {self.size} test episodes in {self.dataset_name}")

    def data_augmentation(self, images):
        images = images / 255
        if self.dataset_name == 'vp2_robodesk_random':
            images = images * 255
        if self.dataset_name == 'tfds_robonet':
            images = F.center_crop(images, min(images.shape[2:]))
        return F.resize(images, self.image_size)
    ...
    def get_segment(self, episode, action=None):
        if 'vp2' in self.dataset_name or self.random_start_frame:
        ...
        # !BUGFIX TODO: ctx_index should be strictly start_index
        if self.random_ctx_frame:
            ctx_index = np.random.randint(max(start-15, 0), start+1)
            images = [episode[ctx_index]] + images
            if action is not None:
...
class EvalDataLoader(data.DataLoader):
    def __init__(self, parent_dir, segment_length, image_size, batch_size=2, num_workers=1, load_action=False, **dummy_args):
        self.dataset = EvalDataset(parent_dir, segment_length, image_size, load_action, **dummy_args)
        super().__init__(self.dataset, batch_size=batch_size, num_workers=num_workers)
