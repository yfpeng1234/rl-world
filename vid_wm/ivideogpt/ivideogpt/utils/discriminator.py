...
class Discriminator3D(ModelMixin, ConfigMixin):
    ...
    def __init__(self, in_channels=3, cond_channels=0, hidden_channels=512, depth=6):
        ...
        layers = [
            nn.utils.spectral_norm(
                nn.Conv3d(in_channels, hidden_channels // (2**d), kernel_size=3, stride=[1, 2, 2], padding=1)
            ),
            nn.LeakyReLU(0.2),
        ]
        for i in range(depth - 1):
            c_in = hidden_channels // (2 ** max((d - i), 0))
            c_out = hidden_channels // (2 ** max((d - 1 - i), 0))
            layers.append(nn.utils.spectral_norm(nn.Conv3d(c_in, c_out, kernel_size=3, stride=[1, 2, 2], padding=1)))
