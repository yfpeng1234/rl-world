...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.kv_pos_emb = nn.parameter.Parameter(torch.zeros((kv_frames * resolution * resolution // 4 * 5, channels)),
                                                 requires_grad=True)
        self.q_pos_emb = nn.parameter.Parameter(torch.zeros((resolution * resolution // 4 * 5, channels)), requires_grad=True)
