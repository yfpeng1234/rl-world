...
class RT1Inference:
    ...
    def _rescale_action_with_bound(
        actions: Union[np.ndarray, tf.Tensor],
        low: float,
        high: float,
    ...
    def _unnormalize_action_widowx_bridge(self, action: dict[str, Union[np.ndarray, tf.Tensor]]) -> dict[str, np.ndarray]:
    ...
    def _resize_image(self, image: Union[np.ndarray, tf.Tensor]) -> tf.Tensor:
    ...
    @staticmethod
    def _small_action_filter_google_robot(raw_action: dict[str, Union[np.ndarray, tf.Tensor]], arm_movement: bool = False, gripper: bool = True) -> dict[str, Union[np.ndarray, tf.Tensor]]:
    ...
    def step(self, image: np.ndarray, task_description: Optional[str] = None) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        """
        Input:
            image: np.ndarray of shape (H, W, 3), uint8    512*640*3
        ...
        save_lists = [[0,0,0], raw_action["gripper_closedness_action"], raw_action["rotation_delta"], raw_action["terminate_episode"], raw_action["world_vector"]]
        save_lists = [np.array(x) for x in save_lists]
        save_lists = np.concatenate(save_lists, axis=0)
        self.history_actions.append(save_lists)
        '''
        terminate_episode: (3,) array([0, 1, 0], dtype=int32)
        base_displacement_vector: (2,) array([-0.00195694, -0.00195694], dtype=float32)
        rotation_delta: (3,) array([ 0.3658018 , -0.00307393,  0.11373663], dtype=float32)
        base_displacement_vertical_rotation: (1,), array([-0.00614786], dtype=float32)
        gripper_closedness_action: (1,), array([-0.00195694], dtype=float32)
        world_vector: (3, ), array([ 0.05088067,  0.1917808 , -0.05088067], dtype=float32)
        '''
        if self.policy_setup == "google_robot":
...
if  __name__ == "__main__":
    rt1_inference = RT1Inference()
    rt1_inference.reset("pick coke can")
    image = np.random.randint(0, 255, (512, 640, 3), dtype=np.uint8)
    raw_action, action = rt1_inference.step(image, "pick up the blue block")
