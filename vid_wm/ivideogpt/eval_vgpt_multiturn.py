...
def parse_args():
    ...
    if args.processor_type == 'ctx_msp':
        args.gen_input_length = args.tokens_per_frame * 16 + args.tokens_per_frame + args.action_dim
        args.gen_output_length = (args.tokens_per_frame + args.action_dim) * (args.segment_length - 1)
    else:
...
def evaluate(args, accelerator, processor, tokenizer, model, eval_dataloader, evaluator, completed_steps=0):
    ...
    for i, batch in enumerate(eval_dataloader):
        ...
        sample_non_repeating_num = [[] for w in range(batch_size)]
        j = -1
        while True:
            j += 1
            # predict next frames
            if args.use_frame_metrics:
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    if args.max_decode_batchsize is not None:
                        recon_output = batch_forward(args.max_decode_batchsize,
                                                        ctx_tokens, output_tokens,
                                                        lambda x,y: accelerator.unwrap_model(tokenizer).detokenize(x, y))
                    else:
                        recon_output = accelerator.unwrap_model(tokenizer).detokenize(
                            ctx_tokens, output_tokens)

                recon_output = recon_output.float().clamp(0.0, 1.0)
            ...
            if len(sample_recon_outputs[0]) == 0 and (not args.reject_repeating or not repeating[0].item()) and args.log_gif and i % args.log_gif_interval == 0 and accelerator.is_main_process and accelerator.distributed_type != DistributedType.FSDP:
                ...
                gt_frames = [(pixel_values[0, k].permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8)
                            for k in range(pixel_values.shape[1])]
                recon_frames = [(recon_output[0, k].permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8)
                                for k in range(recon_output.shape[1])]
                recon_frames = gt_frames[:2] + recon_frames[1:]
            ...
            for w in range(batch_size):
                if not args.reject_repeating or not repeating[w].item():
                    if len(sample_recon_outputs[w]) < args.n_sample:
                        ...
                        if args.reject_repeating:
                            sample_non_repeating_num[w].append(not_repeating_num[w].item())
...
def start_train():
    ...
    model = LLM(model=unwrapped_model_path, gpu_memory_utilization=args.gpu_memory_utilization, enable_prefix_caching=True, dtype='bfloat16')
