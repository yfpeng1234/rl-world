...
from ivideogpt.utils.discriminator import Discriminator
...
def parse_args():
    ...
    parser.add_argument("--segment_length", type=int, default=8,
...
def evaluate(args, accelerator, model, lpips, get_recon_loss, eval_dataloader, global_step):
    ...
    for i, batch in enumerate(eval_dataloader):
        ...
        # compute losses
        module = model.module if accelerator.num_processes > 1 else model
        if args.model_type == 'ctx_vqganv64':
            fmap, fmap_ref, commit_loss, dyna_commit_loss = module(sample=reference_single,
                                                                            dyn_sample=target,
                                                                            return_dict=False,
                                                                            return_loss=True,
                                                                            segment_len=args.segment_length - args.context_length)
        else:
            fmap, commit_loss = module(pixel_values, return_dict=False, return_loss=True)
        ...
        if i % 10 == 0:
            ...
            if 'ctx' in args.model_type:
                if args.context_length > 1:
                    recon[:, :args.context_length * args.resolution[1], :] = ref_recon
                else:
                    recon[:, (args.context_length - 1) *
                            args.resolution[1]:args.context_length * args.resolution[1], :] = ref_recon
...
def main():
    ...
    if args.model_config_name_or_path is None and args.pretrained_model_name_or_path is None:
        if args.model_type == "ctx_vqganv64":
            config = json.load(open("configs/ctx_vae256/config.json"))
            model = CompressiveVQModelFSQ(**config)
        else:
                    ...
                    with torch.no_grad():
                        ...
                        if 'ctx' in args.model_type:
                            if args.context_length > 1:
                                ...
                                recon[:, (args.context_length - 1) * args.resolution[1]:args.context_length *
                                      args.resolution[1], :] = ref_recon
                        diff = np.concatenate([np.zeros_like(context), diff], 1)
                        # error = np.concatenate([np.zeros_like(context), error], 1)
                        error = np.abs(recon - gt)
                        cv2.imwrite(os.path.join(
